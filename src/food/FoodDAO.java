...
public class FoodDAO {
	...
	//饭菜的喜欢数增加1
	public void incLovecount(Integer foodId){
		String sql="update food set food_lovecount=food_lovecount+1 where food_id="+foodId;
		try{
			this.connection.get_statement().executeUpdate(sql);
		}catch(Exception e){
			e.printStackTrace();
	...
	//吃过该饭菜的人数增加1
	public void incEatedcount(Integer foodId){
		String sql="update food set food_eatedcount=food_eatedcount+1 where food_id="+foodId;
		try{
			this.connection.get_statement().executeUpdate(sql);
		}catch(Exception e){
			e.printStackTrace();
	...
	//该饭菜想吃人数增加1
	public void incWanteatcount(Integer foodId){
		String sql="update food set food_wanteatcount=food_wanteatcount+1 where food_id="+foodId;
		try{
			this.connection.get_statement().executeUpdate(sql);
		}catch(Exception e){
			e.printStackTrace();
	...
	//设置food的foodlevel
	public void setFoodLevel(Food food){
		String sql="update food set food_level="+food.getFoodLevel()+" where food_id="+food.getFoodId();
		try{
			this.connection.get_statement().executeUpdate(sql);
		}catch(Exception e){
			e.printStackTrace();
